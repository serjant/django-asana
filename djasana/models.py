...
class BaseModel(models.Model):
    ...
    remote_id = models.BigIntegerField(
        unique=True, db_index=True, help_text=_("The id of this object in Asana.")
    )
...
class Attachment(NamedModel):
    ...
    resource_type = models.CharField(
        max_length=24, null=True, blank=True, default="attachment"
    )
...
class CustomField(NamedModel):
    ...
    precision = models.SmallIntegerField(
        choices=precision_choices, null=True, blank=True
    )
    resource_subtype = models.CharField(
        choices=subtype_choices, max_length=24, null=True, blank=True
    )
    resource_type = models.CharField(
        max_length=24, null=True, blank=True, default="custom_field"
    )
...
class CustomFieldSetting(BaseModel):
    ...
    custom_field = models.ForeignKey(
        "CustomField", to_field="remote_id", on_delete=models.CASCADE
    )
    is_important = models.BooleanField(default=False)
    project = models.ForeignKey(
        "Project", to_field="remote_id", on_delete=models.CASCADE
    )
    resource_type = models.CharField(
        max_length=24, null=True, blank=True, default="custom_field_setting"
    )
    workspace = models.ForeignKey(
        "Workspace", to_field="remote_id", on_delete=models.CASCADE
    )
...
class Project(NamedModel):
    ...
    followers = models.ManyToManyField(
        "User", related_name="projects_following", blank=True
    )
    ...
    resource_type = models.CharField(
        max_length=24, null=True, blank=True, default="project"
    )
    start_on = models.DateField(null=True, blank=True)
    team = models.ForeignKey(
        "Team", to_field="remote_id", null=True, on_delete=models.SET_NULL
    )
    workspace = models.ForeignKey(
        "Workspace", to_field="remote_id", on_delete=models.CASCADE
    )
...
class Story(Hearted, NamedModel):
    ...
    resource_type = models.CharField(
        max_length=24, null=True, blank=True, default="story"
    )
...
class Tag(NamedModel):
    ...
    notes = models.TextField(null=True, blank=True)
    resource_type = models.CharField(
        max_length=24, null=True, blank=True, default="tag"
    )
    workspace = models.ForeignKey(
        "Workspace",
        to_field="remote_id",
...
class Task(Hearted, NamedModel):
    ...
    resource_type = models.CharField(
        max_length=24, null=True, blank=True, default="task"
    )
    start_on = models.DateField(null=True, blank=True)
    ...
    def refresh_from_asana(self):
        ...
        if task_dict["assignee"]:
            user = User.objects.get_or_create(
                ...
                defaults={"name": task_dict["assignee"]["name"]},
            )[0]
        ...
        for tag_ in tags_dict:
            tag = Tag.objects.get_or_create(
                remote_id=tag_["gid"], defaults={"name": tag_["name"]}
            )[0]
            self.tags.add(tag)
        if dependencies:
    ...
    def get_custom_fields(self):
        ...
        for custom_field in response:
            if custom_field["resource_subtype"] == "enum":
                custom_field_values[custom_field["name"]] = custom_field["enum_value"][
                    "name"
                ]
            elif custom_field["resource_subtype"] == "number":
                if custom_field.get("precision", 0):
                    custom_field_values[custom_field["name"]] = float(
                        custom_field["number_value"]
                    )
                else:
                    custom_field_values[custom_field["name"]] = int(
                        custom_field["number_value"]
                    )
            else:
...
class Team(NamedModel):
    ...
    resource_type = models.CharField(
        max_length=24, null=True, blank=True, default="team"
    )
...
class User(NamedModel):
    ...
    resource_type = models.CharField(
        max_length=24, null=True, blank=True, default="user"
    )
...
class Webhook(models.Model):
    ...
    resource_type = models.CharField(
        max_length=24, null=True, blank=True, default="webhook"
    )
...
class Workspace(NamedModel):
    ...
    resource_type = models.CharField(
        max_length=24, null=True, blank=True, default="workspace"
    )
